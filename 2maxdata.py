...
1. **Scrape Data**: Retrieves race results for Max Verstappen from the Formula 1 website for each year from 2007 to 2024.
2. **Data Processing**: Extracts relevant information and stores it in a pandas DataFrame.
3. **Database Insertion**: Inserts the consolidated data into a MySQL database table named `maxdata`.
'''
...
for year in range(2007, 2025):
    ...
    post = requests.get(url=f'https://www.formula1.com/en/results/{year}/drivers/MAXVER01/max-verstappen')
...
# Insert the consolidated DataFrame into the MySQL database table 'maxdata'
all.to_sql('maxdata', con=con, index=False)
